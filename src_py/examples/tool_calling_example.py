#!/usr/bin/env python
# Copyright 2025 Prism Shadow. and/or its affiliates
#
# Licensed under the Apache License, Version 2.0 (the "License");
# you may not use this file except in compliance with the License.
# You may obtain a copy of the License at
#
#     http://www.apache.org/licenses/LICENSE-2.0
#
# Unless required by applicable law or agreed to in writing, software
# distributed under the License is distributed on an "AS IS" BASIS,
# WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
# See the License for the specific language governing permissions and
# limitations under the License.

"""
Example demonstrating tool calling with multi-turn conversation.

This example shows how to use function calling with AutoLLMClient to query weather information.
"""

import asyncio
import json

from agent_adapter import AutoLLMClient


def get_current_temperature(location: str) -> str:
    """Mock function to get current temperature for a location."""
    # Mock weather data
    weather_data = {
        "London": "15°C",
        "San Francisco": "18°C",
        "Tokyo": "22°C",
        "New York": "12°C",
        "Paris": "14°C",
    }
    return weather_data.get(location, "20°C")


async def main():
    """Example of tool calling with AutoLLMClient."""
    print("=" * 60)
    print("Tool Calling Example")
    print("=" * 60)

    # Define the function declaration for the model
    weather_function = {
        "name": "get_current_temperature",
        "description": "Gets the current temperature for a given location.",
        "parameters": {
            "type": "object",
            "properties": {
                "location": {
                    "type": "string",
                    "description": "The city name, e.g. San Francisco",
                },
            },
            "required": ["location"],
        },
    }

    client = AutoLLMClient(model="gemini-3-flash")
    config = {"tools": [{"function_declarations": [weather_function]}]}

    # First turn: User asks about temperature
    print("User: What's the temperature in London?")
    print("Assistant: ", end="", flush=True)

    events = []
    async for event in client.streaming_response_stateful(
        message={"role": "user", "content_items": [{"type": "text", "text": "What's the temperature in London?"}]},
        config=config,
    ):
        events.append(event)
        if event["content_items"]:
            for item in event["content_items"]:
                if item.get("type") == "text":
                    print(item.get("text", ""), end="", flush=True)

    print()

    # Check if there's a function call in the last event
    function_call = None
    for event in events:
        for item in event.get("content_items", []):
            if item.get("type") == "function_call":
                function_call = item
                break
        if function_call:
            break

    if function_call:
        print(f"\nFunction to call: {function_call.get('name')}")
        print(f"Arguments: {function_call.get('argument')}")

        # Call the function
        args = json.loads(function_call.get("argument", "{}"))
        result = get_current_temperature(**args)
        print(f"Function result: {result}")

        # Second turn: Send function result back to the model
        print("\nSending function result back to model...")
        print("Assistant: ", end="", flush=True)

        async for event in client.streaming_response_stateful(
            message={
                "role": "tool",
                "content_items": [{"type": "text", "text": result}],
                "tool_call_id": function_call.get("name", ""),
            },
            config=config,
        ):
            if event["content_items"]:
                for item in event["content_items"]:
                    if item.get("type") == "text":
                        print(item.get("text", ""), end="", flush=True)

        print()

        # Third turn: Ask a follow-up question
        print("\nUser: How about in Paris?")
        print("Assistant: ", end="", flush=True)

        events2 = []
        async for event in client.streaming_response_stateful(
            message={"role": "user", "content_items": [{"type": "text", "text": "How about in Paris?"}]},
            config=config,
        ):
            events2.append(event)
            if event["content_items"]:
                for item in event["content_items"]:
                    if item.get("type") == "text":
                        print(item.get("text", ""), end="", flush=True)

        print()

        # Check for another function call
        function_call2 = None
        for event in events2:
            for item in event.get("content_items", []):
                if item.get("type") == "function_call":
                    function_call2 = item
                    break
            if function_call2:
                break

        if function_call2:
            print(f"\nFunction to call: {function_call2.get('name')}")
            print(f"Arguments: {function_call2.get('argument')}")

            # Call the function again
            args2 = json.loads(function_call2.get("argument", "{}"))
            result2 = get_current_temperature(**args2)
            print(f"Function result: {result2}")

            # Send result back
            print("\nSending function result back to model...")
            print("Assistant: ", end="", flush=True)

            async for event in client.streaming_response_stateful(
                message={
                    "role": "tool",
                    "content_items": [{"type": "text", "text": result2}],
                    "tool_call_id": function_call2.get("name", ""),
                },
                config=config,
            ):
                if event["content_items"]:
                    for item in event["content_items"]:
                        if item.get("type") == "text":
                            print(item.get("text", ""), end="", flush=True)

            print()

    print("\n" + "=" * 60)
    print("Conversation complete!")
    print("=" * 60)


if __name__ == "__main__":
    asyncio.run(main())
