...
- If a model finishes a turn with thinking output only (no other content and no tool calls), AgentHub throws `EmptyResponseError`, because sending that message back on the next turn fails with a 400 error; retry or re-prompt the model. It and `ToolCallArgumentParseError` inherit `AgentHubError`.
