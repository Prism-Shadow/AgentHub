...
Use exact model IDs. If a model ID is not listed, ask the user to confirm the exact ID before using it. The supported `(model, base_url, client)` triples are also available at runtime via `listSupportedModels()`.
