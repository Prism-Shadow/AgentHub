...
- **No helper layer.** Module-level helpers for field access, JSON normalization or validation, error formatting, or usage arithmetic are out. SDK attributes are read directly, usage arithmetic is inlined, and dict/attribute dual-access shims are never warranted.
