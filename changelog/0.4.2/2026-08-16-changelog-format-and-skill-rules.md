...
- Unreleased work is pinned to one folder: the single release folder whose README title carries `(unreleased)`, found with `grep -l '(unreleased)' changelog/*/README.md`. Nothing may open a second folder for unshipped work or invent the next version number.
