...
- Unreleased work is pinned to `changelog/unreleased/`, a folder named for its state rather than a number, because the version is not decided until release. Nothing may create a numbered folder for unshipped work or invent the next version number; release preparation renames `unreleased/` to the decided version.
