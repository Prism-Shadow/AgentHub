...
- **由可读性决定代码放在哪里。** 读者应当能顺着一条请求或响应把文件一路读下来，因此默认内联：直接读取 SDK 属性，字段访问、用量算术、错误文案都留在使用它们的地方。辅助函数只用于确实很大、且自成一体、内联会淹没主流程的代码块——比如抓取并解码图片——绝不用于几行代码，也绝不在参考客户端自有的私有方法旁边另起一层。同时兼容 dict 与 SDK 对象的双路访问 shim 在任何情况下都不成立，因为事件本身是带类型的。
