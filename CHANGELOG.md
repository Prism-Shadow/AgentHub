...
- [2026-08-19] [Version 0.4.5](changelog/0.4.5/README.md): streaming clients skip the events a gateway injects from outside the protocol, including a heartbeat spelled the way another protocol spells it, and still raise on unknown events inside the protocol; `AutoLLMClient` lists the model ids an endpoint serves, and every client accepts default headers for endpoints that demand their own.
