...
| [reasoning\_effort](#reasoning_effort) | String  | `max`                 | Controls the model's reasoning effort level, only supported by `GLM-5.2` and above.           |
...
* `type` (string):
  * `enabled` (default): Enable chain of thought. `GLM-5.2`, `GLM-5.1`, `GLM-5`, `GLM-5-Turbo`, `GLM-5V-Turbo`, `GLM-4.6`, and `GLM-4.5` auto-determine whether to think, while `GLM-4.7` and `GLM-4.5V` use forced thinking.
...
* Allowed values: `max`, `xhigh`, `high`, `medium`, `low`, `minimal`, `none`
* Default: `max`
...
* `xhigh`, `medium`, `low`, `minimal`, `none`: Compatibility mappings to maintain compatibility with other protocols

Notes:

* For compatibility with other protocols, passing `none` or `minimal` will cause the model to skip thinking; `low` and `medium` will be mapped to `high`; `xhigh` will be mapped to `max`.
