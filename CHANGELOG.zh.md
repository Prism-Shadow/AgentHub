...
- [2026-08-19] [版本 0.4.5](changelog/0.4.5/README.zh.md)：流式 client 跳过网关从协议之外注入的事件（包括按另一种协议拼法命名的心跳），协议内部的未知事件仍然抛出；`AutoLLMClient` 可以列出 endpoint 提供的模型 id，且全部 client 支持传入默认 header 以对接要求特定 header 的 endpoint。
