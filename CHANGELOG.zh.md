...
- [2026-08-19] [版本 0.4.5](changelog/0.4.5/README.zh.md)：流式 client 默认静默跳过自己不认识的流式输出，`AGENTHUB_DEBUG` 开启时才抛出；`AutoLLMClient` 可以列出 endpoint 提供的模型 id，且全部 client 支持传入默认 header 以对接要求特定 header 的 endpoint。
